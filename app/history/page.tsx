...
export default async function HistoryPage({
  ...
  // Group tasks by student, preserving updatedAt desc order within each group
  ...
  for (const task of tasks) {
    if (!groupMap.has(task.studentId)) {
      groupMap.set(task.studentId, {
    ...
    groupMap.get(task.studentId)!.tasks.push(task);
  }

  // Sort student groups: most recently updated task first
  ...
  return (
    <div>
      ...
      {groups.length === 0 ? (
        ...
        <>
          <p className="text-sm text-gray-500 mb-4">{groups.length} student{groups.length !== 1 ? "s" : ""} · {tasks.length} task{tasks.length !== 1 ? "s" : ""}{status || studentId ? " (filtered)" : ""}</p>
          <div className="space-y-6">
            {groups.map((g) => (
              <div key={g.studentId} className="rounded-xl overflow-hidden" style={{ border: "1px solid #afa9ec", boxShadow: "0 1px 4px rgba(61,44,141,0.08)" }}>
                <SectionHeader title={`${g.lastName}, ${g.firstName} — Year ${g.year}`} />
